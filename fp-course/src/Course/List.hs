...
filter ::
  ...
  List a ->
  List a
filter f = foldRight (\e x -> if f e then e :. x else x) Nil
...
seqOptional =
  ...
  where
    f :: Optional a -> Optional (List a) -> Optional (List a)
    -- f = twiceOptional (:.)
...
--   ( \xs x ->
--       case xs of
--         Empty -> if f x then Full x else Empty
--         Full _ -> xs
--   )
--   Empty
...
reverse ::
  ...
  List a
reverse Nil = Nil
reverse (x :. xs) = foldLeft (flip (:.)) (x :. Nil) xs
...
-- https://github.com/tonymorris/fp-course/blob/73a64c3d4d0df58654a1a9a0ee9d9b11c3818911/src/Course/List.hs#L339
